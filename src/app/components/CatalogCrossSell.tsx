...
export function CatalogCrossSell({ crossSell }: CatalogCrossSellProps) {
  return (
    ...
    >
      <div className="px-4 lg:px-8 pt-12 pb-6 flex items-end justify-between bg-(--banner-bg)">
        <div>
      ...
      <div className="grid gap-px bg-white overflow-x-auto scrollbar-hide"
        ...
        {crossSell.categories.map(cat => (
          <Link key={cat.label} href={cat.href ?? crossSell.href}
            className="group cursor-pointer no-underline bg-white flex flex-col outline-1 outline-white min-w-40">
            <div className="relative overflow-hidden aspect-3/4">
              <Image src={cat.image} alt={cat.label} fill
                sizes="(max-width: 640px) 50vw, 25vw"
                className="object-cover object-center transition-transform duration-500 group-hover:scale-105" />
            </div>
            <div className="px-3 pt-3 pb-4 bg-white flex-1 flex flex-col justify-between min-h-18">
