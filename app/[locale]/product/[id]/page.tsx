...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  // Brand name, currency and the delivery promise quoted in the description are editor-owned, and must match the offer the same page publishes as structured data further down.
...
export default async function Page({ params }: Props) {
  ...
  const genderHref = await (async () => {
    ...
    // `clothing` is the landing every other gender link in the storefront uses (`PAGE_REGISTRY`, the empty-cart CTA, the store locator …).
  ...
  // Aggregate rating computed from OE product reviews.
  ...
  // Structured data mirrors the visible breadcrumb: the same category chain, with each crumb's own url where the storefront has one.
  ...
  // Reviews and the "You May Also Like" carousel each get their own Suspense boundary so the main PDP body renders immediately.
  const categoryViewAllHref = categoryPathToViewAllHref(productCategory);
  // Full OE category path (e.g. `/women/women_clothing/outerwear`).
  const productCategoryPath = productCategory;
  // Effective gender for the recommendations filter.
  ...
  // `reserve_in_store` is the one OE form that only this route renders, so its content is loaded here instead of the root layout.
