...
Keys read from `catalog_page` (all optional, each falls back to `data/catalogPageLabels.ts`): `catalog_page_women`, `catalog_page_men`, `catalog_page_clothing`, `catalog_page_shoes`, `catalog_page_bags`, `catalog_page_accessories`, `catalog_page_breadcrumb_home`, `catalog_page_breadcrumb_women`, `catalog_page_breadcrumb_men`, `catalog_page_breadcrumb_clothing`, `catalog_page_breadcrumb_shoes`, `catalog_page_breadcrumb_bags`, `catalog_page_breadcrumb_accessories`.
...
### 5.6 System text (`src/lib/oneentry/system-text.ts`)

Exports: `Lang` type (currently `'en_US'`), `SystemSchema` type, `readSystemValue(set, path)` (untyped accessor), `getSystemSet(marker, lang)` (cached attribute-set loader — 5-minute TTL + React cache), `t(set, path)` (dot-path accessor with safe fallback).
...
- **Next.js ISR** — `src/lib/isr.ts` exports revalidate constants via a `ttl(envKey, fallback)` helper. These constants are consumed **only** by `unstable_cache`-wrapped loader functions; they are not imported by route shells. Route-shell `export const revalidate` must be a statically-analysable literal — importing a computed value causes Next.js to throw "Invalid segment configuration export detected" and abort the build. The four ISR route shells therefore declare plain numeric literals: `app/page.tsx` → `300`, `app/product/[id]/page.tsx` → `120`, `app/[...slug]/page.tsx` → `60`, `app/stores/page.tsx` → `3600`. The `ISR_*_TTL_SEC` env vars tune only the loader-level `unstable_cache` TTLs, not the route-shell revalidate windows. Set `ISR_DISABLED=1` to collapse all loader TTLs to 1 s (has no effect on route-shell literals).
  ...
  **Next.js 16 gotcha — `generateStaticParams` required for ISR on dynamic segments.** A route with a `[param]` segment that does not export `generateStaticParams` is classified as fully dynamic regardless of `revalidate`: the value is silently ignored and every request re-SSRs. `app/product/[id]/page.tsx` exports `export async function generateStaticParams() { return []; }` alongside `revalidate = 120`; together they enable on-demand ISR — the first request for a given `id` generates and caches the HTML, all subsequent requests within the 120 s window are served from the Next.js Data Cache. The empty array means no ids are pre-built at deploy time. This export is load-bearing — removing it reverts the route to fully dynamic even though `revalidate` is still present.
  ...
  | --- | --- | --- | --- | --- |
  | `REVALIDATE_HOME` | 300 s | `ISR_HOME_TTL_SEC` | `loadHeroSlides`, `loadHomepageCollections`, `loadDiscountBanner`, `loadCategorySection`, the `page-blocks.ts` loaders, `cachedFetchFormData` (reviews) | `app/page.tsx` → `300` |
  | `REVALIDATE_PRODUCT` | 120 s | `ISR_PRODUCT_TTL_SEC` | `cachedGetProductById`, `cachedGetRelated`, `cachedGetByIds` | `app/product/[id]/page.tsx` → `120` |
  | `REVALIDATE_CATALOG` | 60 s | `ISR_CATALOG_TTL_SEC` | `cachedProductList`, `loadPurchaseBonusRuleCached`, `loadProductDiscountsCached` | `app/[...slug]/page.tsx` → `60` |
  | `REVALIDATE_STORES` | 3600 s | `ISR_STORES_TTL_SEC` | `loadStores`, `loadCheckoutSuccessMessage`, `loadDeliveryMethodInfo`, `loadDeliverySchedule` | `app/stores/page.tsx` → `3600` |

  `/sale` and `/new` are not in the table: their loaders (`loadSalePage`, `loadNewArrivalsPage`) hold a literal `revalidate: 60` rather than a shared constant, so `ISR_DISABLED` does not reach them. The former `REVALIDATE_SALE` / `REVALIDATE_NEW` / `REVALIDATE_INFO` constants were removed — nothing imported them, so the matching `ISR_SALE_TTL_SEC` / `ISR_NEW_TTL_SEC` / `ISR_INFO_TTL_SEC` env vars did nothing when set.
...
- `system-text.ts` + `system-text.test.ts`
...
| `app/auth/callback/google/page.tsx` | GET handler for Google's OAuth redirect (`?code=&state=&error=`). Calls `exchangeGoogleCodeAction` then `redirect()`s to `returnTo` on success or to `/?googleAuthError=<code>` on failure. |
...
| `src/app/pages/PaymentPage.tsx` | Calls `getPaymentAccountsAction` + `createOrderAction` |
