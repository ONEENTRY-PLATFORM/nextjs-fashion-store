...
The catalog branch of `app/[...slug]/page.tsx` additionally calls `loadPageBlocksByUrl(catalogKey.replace(/-/g, '_'))` (e.g. `women_clothing`, `men_shoes`) and forwards the resulting `pageBlocks` to the catalog page component, which passes them through to `<CatalogTemplate>`. Before forwarding, the shell filters out any block whose marker is `catalog_trend_blocks` — that marker is loaded and rendered separately as the gender-scoped `trendingBlock` via `getCachedTrending`; without this filter it would appear twice. `<PageBlocksRenderer>` is placed at the bottom of `<main>`, after `<CatalogCrossSell>` and the `trendingBlock` slot, just before the closing `</main>` tag — CMS-attached blocks are treated as supplementary content (promos, cross-sells, recently-viewed rails) that appends below the primary catalog UI.
...
| `src/app/components/catalog/CatalogTemplate.tsx` | Main catalog engine (grid, filters, sort, pagination) |
| `src/app/components/catalog/CatalogTemplate.parts.tsx` | Filter drawer pieces, sort button, view mode toggle |
| `src/app/components/catalog/MobileFilterPanel.tsx` + `MobileFilterBody.tsx` | Mobile full-screen filter accordion |
| `src/app/components/catalog/CatalogMobileSort.tsx` | Mobile sort picker |
| `src/app/components/catalog/NoFilterResults.tsx` | Empty state |
| `src/app/components/product/ProductCard.tsx` | Grid card |
| `src/app/components/catalog/CatalogListProductCard.tsx` | List-view card variant |
...
- `HomePage.tsx` (client) now delegates block rendering to the shared `<PageBlocksRenderer>` component (`src/app/components/blocks/PageBlocksRenderer.tsx`) instead of an inline `switch`. The renderer handles `hero_slider → HeroSlider`, `category_section → CategorySection`, `promo_block → PromoBlock`, `discount_banner → DiscountBanner`, `men_collection` / `homepage_best_sellers → MenCollection`, `women_collection` / `homepage_new_arrivals → WomenCollection`, `new_arrivals` / `homepage_sale → NewArrivals`. In addition, blocks are also matched **by `block.type`**: `recently_viewed_block` renders `<RecentlyViewedSection>` driven by the Redux `recentlyViewed.items` trail (hidden when the trail is empty). Any unrecognised marker with a non-empty `products` list falls back to `NewArrivals`. Blocks with a `title` but no `products` and no dedicated component render a plain `<h2>` header so admin-attached blocks are never silently invisible — **except** `cart_complement_block` with an empty `products` array, which returns `null` (a header-only cross-sell block is storefront noise; if OE later inlines products the non-empty `products` branch catches it automatically). Completely empty blocks (no title, no products, no matched type) also render `null`. The homepage passes `initialHeroSlides`, `initialPromoItems`, `initialDiscountBanner`, and `initialCategorySection` as pre-fetched props; other pages omit these and the child components fetch client-side on mount.
- **`AnimatedSection`** (internal component, top of `PageBlocksRenderer.tsx`) wraps **only** the marker-matched homepage blocks: `hero_slider`, `category_section`, `promo_block`, `discount_banner`, `men_collection` / `homepage_best_sellers`, `women_collection` / `homepage_new_arrivals`, `new_arrivals` / `homepage_sale`. It starts `visible = immediate`, mounts an `IntersectionObserver` (`threshold: 0.05`, `rootMargin: '0px 0px -40px 0px'`) that flips `visible = true` on first intersection, then disconnects. The transition class is `transition-[opacity,transform] duration-[650ms] ease-out`, moving `opacity-0 translate-y-7 → opacity-100 translate-y-0`. The hero passes `immediate` so it renders at full opacity on first paint. A `pageshow` listener force-reveals when `event.persisted` is true (bfcache safety net). **Default-branch blocks** (`recently_viewed_block`, generic product-list fallback, title-only fallback) are **not** wrapped in `AnimatedSection` — they render as bare `<div>` / `<section>` and are visible from mount. This prevents below-fold blocks on checkout and catalog pages (≥ 1500 px from top) from staying at `opacity: 0` when users have no reason to scroll. **Note:** the current implementation does not read `sessionStorage['homepageAnimated']` in `AnimatedSection` — the key is still declared in `STORAGE_KEYS.HOMEPAGE_ANIMATED` (value `'homepageAnimated'`, `constants/timings.ts`) and is only consumed by `CategorySection` (see below).
...
- Route shell `app/cart/page.tsx` calls `loadPageBlocksByUrl('cart')` and forwards the result as `pageBlocks` to `<CartPage pageBlocks={pageBlocks} />`. `<PageBlocksRenderer blocks={pageBlocks} />` is placed after the main cart content and before `<Footer>` — the same bottom-of-page convention as catalog and info routes.

**`/checkout/delivery` (`DeliveryPage`)**

- Route shell `app/checkout/delivery/page.tsx` calls `loadPageBlocksByUrl('delivery_method')` (note: the OE page marker is `delivery_method`, not `checkout`) and forwards the result as `pageBlocks` to `<DeliveryPage pageBlocks={pageBlocks} />`. `<PageBlocksRenderer>` is placed after the main delivery-step content, before `<Footer>`.

**`/checkout/payment` (`PaymentPage`)**

- Route shell `app/checkout/payment/page.tsx` calls `loadPageBlocksByUrl('payment')` and forwards the result as `pageBlocks` to `<PaymentPage pageBlocks={pageBlocks} />`. `<PageBlocksRenderer>` is placed after the payment form, before `<Footer>`.

**`/stores` (`StoreLocationsPage`)**

- ISR route: `app/stores/page.tsx` uses `export const revalidate = 3600` (hard-coded literal — all route-shell revalidate values must be literals). Shell awaits **four** loaders in parallel: `loadStoresSystemTexts()`, `loadStores()`, `loadStoreLocationsPage()`, `loadPageBlocksByUrl('stores')` — the last is forwarded to `<StoreLocationsPage pageBlocks={pageBlocks}>` so CMS-attached blocks are passed to `<StoreLocationsPage>` and rendered below the closing `</main>` tag (after the full store grid), immediately before `<Footer>`. Wraps the page in `<StoresLabelsProvider>` and emits **one ClothingStore JSON-LD blob per store** via `buildStoreSchema`. Each schema includes `image`, `PostalAddress`, `telephone`, `email`, `openingHoursSpecification` (day labels mapped through `SCHEMA_DAYS` — `Mon–Sat` expands to six day entries), `hasMap`, `currenciesAccepted`, `paymentAccepted`, `priceRange` (defaults from `STORE_SCHEMA_DEFAULTS`).
