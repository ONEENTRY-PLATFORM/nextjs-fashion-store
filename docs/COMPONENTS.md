...
| `CatalogTemplate.types.ts` | Shared types: `FilterGroup`, `FilterOption`, `TrendBlock`, `CrossSellCategory`, `BreadcrumbItem`. | — |
| `CatalogListProductCard.tsx` | List-view (compact) variant of the product card — used when `state.catalog[key].listMode === true`. | — |
| `CatalogTrendBlocks.tsx` | Renders trend category blocks from `data/trendBlocks.ts`. | — |
...
| `ShoesCatalog.tsx` | Catalog wrapper for shoe pages — passes shoes-specific `FILTER_GROUPS` / `QUICK_CHIPS` / accent into `<CatalogTemplate>`. Re-exports `FilterOption`, `FilterGroup`, `TrendBlock`, `CrossSellCategory` types for use by page shells. Also used as the delegate target for `<AccessoriesCatalog>`. | — |
...
| `LoginModal.tsx` | Sign-in form: single login field (accepts email, phone, or OE identifier — the OE email auth provider has `isLogin: true` on all three) + password + Google button + "Forgot password?" (stub, `alert()`) + link to `RegisterModal`. Google flow: imports **`requestGoogleIdToken`** from `src/lib/google-auth.ts` (back-compat alias for the underlying `requestGoogleAccessToken()`) — runs client-side to open the GIS popup and returns an `access_token`. Hands off to `useAuth().loginWithGoogle(token)` (which internally awaits `signInWithGoogleAction`). Consumes `useSignInT()` for CMS labels; falls back to `AUTH_LABELS`. | [AUTH.md §4](./AUTH.md#4-sign-in) |
| `RegisterModal.tsx` | Sign-up form rendered from `SignUpFormSchemaContext` (CMS attribute set `users_sign_in_sign_up`). Zod client validation via `registerSchema.safeParse()`. Calls **`useAuth().signUp(input)`** (context method), which internally awaits `signUpAction`. Google OAuth imports `requestGoogleIdToken` (back-compat alias) — returns an access_token — passed to `useAuth().loginWithGoogle(token)`. Consumes `useCreateAccountT()` labels. | [AUTH.md §5](./AUTH.md#5-sign-up) |
...
| `myData/SocialNetworksSection.tsx` | Google linking → `connectGoogleAccountAction`; Apple/Facebook are visual stubs. | [ACCOUNT.md §2](./ACCOUNT.md#2-my-data-mydatasectiontsx) |
