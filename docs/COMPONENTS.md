...
| `Footer.tsx` | Link columns, payment icons (`PAYMENT_METHOD_NAMES`), social links + support cards + company blurb/phone/copyright via `useFooterT` (OE `footer` set, `footerConfig.ts` as fallback), bottom bar. Embeds `<NewsletterForm>`. **Both navigation halves come from `useFooterMenu()`** (OneEntry `footer` menu via `FooterMenuContext`), split by `menus/adapt-footer.ts`: nested root nodes → columns (`footerColumnsFromMenu`), childless root nodes → bottom bar (`footerBottomLinksFromMenu`). `FOOTER_LINKS` / `BOTTOM_LINKS` in `data/footerConfig.ts` are the per-half fallbacks; branding copy still comes from that file + `FOOTER_LABELS`. | — |
...
| `CatalogCrossSell.tsx` | Bottom-of-catalog cross-sell grid (`CrossSellCategory[]` — e.g. "Also popular in Men's Bags"). Copy from its own `CATALOG_CROSS_SELL_LABELS`, overlaid via `interface_controls_view_`. | — |
...
| `ColorSwatchButton.tsx` | Button variant used inside ProductCard's hover swatch row and add-to-bag flow. Uses `strikeColor()` (`utils/colorUtils.ts`) to render the OOS diagonal-strike overlay with the right contrast against the swatch. Copy from its own `COLOR_SWATCH_BUTTON_LABELS`, overlaid via `interface_controls_view_`. | — |
...
| `InfoPage.tsx` (198 loc) | Rendered by `app/[...slug]/page.tsx` when the resolved entry is an info page and there is no dedicated section body. It does **not** call `loadPageByUrl` — the content comes from local static tables (`INFO_PAGE_LABELS`, `INFO_PAGE_HERO`, `INFO_PAGE_CTA`, `INFO_PAGE_SECTIONS`, `INFO_PAGE_FEATURE_CARDS`, `INFO_PAGE_DEMO_NOTICE`) declared in `InfoPage.tsx` itself. Icon map covers `edit`/`layout`/`zap`/`globe`. | [pages/info-page.md](./pages/info-page.md) |
...
| `SalePage.tsx` (479 loc) | `/sale` | — | Countdown + category tabs + biggest-discount sort. Props: `{initialProducts, saleEndsAt, gender?, pageBlocks?, cmsPage?}`. `cmsPage` is a `SalePageFromCms` value returned by `loadSalePage()` and forwarded as `cms` to `<SaleHero>`. The mid-promo block reads `cmsPage?.promo.*` with `L.promo*` static fallbacks. `saleEndsAt` is sourced from `cmsPage.saleEndsAt` (OE `page_sale_top_banner_timer`), with `SALE_END_DATE` as the hard-coded fallback. **Recs section:** finds the first `pageBlocks` entry with `type === 'frequently_ordered_block'`; uses `block.title` as the heading and `block.products` when non-empty, falling back to `PRODUCTS.slice(0, 6)` (OE's `frequently_ordered_block` returns no products without a seed `productId` context). That block is then filtered out of the list forwarded to `<PageBlocksRenderer>` so it does not also render a duplicate heading stub at the bottom of the page. |
...
| `myData/SocialNetworksSection.tsx` | Google linking → `startGoogleOAuth('/account?googleLinked=1')` (browser redirects to Google; the callback client page runs the exchange, which links the Google identity to the current user because the SDK still holds the session). Mount effect reads `?googleLinked=1` to mark Google as linked. Apple/Facebook are visual stubs. | [ACCOUNT.md §2](./ACCOUNT.md#2-my-data-mydatasectiontsx) |
...
- **Global components (`src/app/components/*.tsx`)** — 49/49 covered. (The `figma/` subfolder has been removed — `figma/ImageWithFallback.tsx` was deleted as a dead duplicate.)
