...
- **Neutral colors: not followed.** ~19 components carry arbitrary hex for greys, borders and warning colors — `border-[#e5e7eb]`, `bg-[#fafafa]`, `text-[#78350f]` ([`CartUnavailableNotice.tsx:29-44`](../src/app/components/CartUnavailableNotice.tsx#L29-L44), [`CatalogMobileSort.tsx:36-54`](../src/app/components/CatalogMobileSort.tsx#L36-L54)). These have no semantic token yet; add one before reaching for a literal.
