...
| `src/app/actions/` | Only `auth.ts::validateCredentials` — legacy fallback (kept to satisfy the older login modal path). Not used on the main OneEntry path. |
| `src/app/constants/` | `colors.ts` (accent palette), `timings.ts` (delays, TTLs, limits). |
| `src/app/data/` | ~40 static TypeScript datasets — remaining labels, SEO metadata, config, `pageRegistry`, `cms-product-id-map` (playground ↔ Platform id bridge). See [DATASETS.md](./DATASETS.md). |

### `src/lib/` — SDK + external integrations

| Folder / file | Purpose |
|---|---|
| `src/lib/google-auth.ts` | Google Identity Services helper — lazy-loads the GIS script, opens the token-client popup, resolves with a Google `access_token` that gets forwarded to `signInWithGoogleAction` / `connectGoogleAccountAction`. Env: `NEXT_PUBLIC_GOOGLE_CLIENT_ID`. |
...
| `src/lib/oneentry/auth/` | All auth Server Actions (`signInAction`, `signInWithGoogleAction`, `signUpAction`, `signOutAction`, `getCurrentUserAction`, `updateProfileAction`, `updateAddressesAction`, `updateSubscriptionsAction`, `updateConsentAction`, `syncCartAction`, `syncWishlistAction`, `createOrderAction`) + cookie management (`oe_access`, `oe_refresh`, `oe_user`). Also `sign-up-form.ts` — attribute-set schema for the registration form and its React context. |
...
| `src/lib/oneentry/blocks/` | Homepage block loaders: `hero-slides` (marker `hero_slider`), `homepage-collections` (`homepage_collections`), `discount-banner` (`discount_banner`), `homepage-product-blocks`, `category-section` (`category_section`), `page-blocks` (loads block IDs from a page, resolves product blocks + frequently-ordered rules; when a home-page `similar_products_block` returns no items — no seed product for OE's similarity engine — `loadHomepageBlockFallback` maps the known markers `homepage_new_arrivals` / `homepage_best_sellers` / `homepage_sale` to product labels `NEW` / `BESTSELLER` / `SALE` and pulls tagged products via `loadProducts({ tags: [label] })`; unknown markers stay empty so the block hides silently). |
...
| Context | Holds |
|---|---|
| `AuthContext` | `isLoggedIn`, `user` (merged from `getCurrentUserAction`), `authReady` flag (false until bootstrap `/me` completes), login/register modal state, and 11 mutation callbacks that delegate to `src/lib/oneentry/auth/actions.ts` (`login`, `loginWithGoogle`, `signUp`, `logout`, `updateUser`, `updateProfile`, `updateAddresses`, `updateSubscriptions`, `updateConsent`, `syncCart`, `syncWishlist`). The remaining 7 auth Server Actions (`connectGoogleAccount`, `getCart` / `getWishlist`, `pushRecentlyViewed` / `getRecentlyViewed` / `mergeRecentlyViewed`, `createOrder`) are imported and called directly from components. |
