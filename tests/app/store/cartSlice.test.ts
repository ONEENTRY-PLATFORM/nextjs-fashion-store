...
describe('cartSlice', () => {
  ...
  describe('addItem', () => {
    ...
    it('clamps merged quantity to stockLimit even when colors match', () => {
      ...
      let state = cartReducer(
        emptyState,
        cartActions.addItem(makeItem({ size: 'S', color: 'Red', quantity: 3, stockLimit: 4 })),
      );
      state = cartReducer(
        state,
        cartActions.addItem(makeItem({ size: 'S', color: 'Red', quantity: 3, stockLimit: 4 })),
  ...
  describe('addBundle', () => {
    it('adds multiple items sharing the same bundleId', () => {
      const items = [makeItem({ id: 'b1', size: 'S' }), makeItem({ id: 'b2', size: 'M' })];
  ...
  describe('stockLimit cap — addItem', () => {
    it('clamps initial quantity to stockLimit on first add', () => {
      const state = cartReducer(emptyState, cartActions.addItem(makeItem({ quantity: 10, stockLimit: 5 })));
      expect(state.items[0].quantity).toBe(5);
    });

    it('clamps accumulated quantity to stockLimit when adding to existing item', () => {
      let state = cartReducer(emptyState, cartActions.addItem(makeItem({ quantity: 3, stockLimit: 5 })));
      state = cartReducer(state, cartActions.addItem(makeItem({ quantity: 3, stockLimit: 5 })));
      expect(state.items[0].quantity).toBe(5);
    });
    ...
    it('refreshes stockLimit and re-clamps existing quantity when incoming payload carries a new limit', () => {
      ...
      let state = cartReducer(emptyState, cartActions.addItem(makeItem({ quantity: 5, stockLimit: 10 })));
      // Incoming add: stockLimit=3 — inventory shrank; quantity must be clamped to 3
      state = cartReducer(state, cartActions.addItem(makeItem({ quantity: 0, stockLimit: 3 })));
      expect(state.items[0].stockLimit).toBe(3);
      expect(state.items[0].quantity).toBe(3);
    });

    it('preserves existing stockLimit when incoming payload has stockLimit === undefined', () => {
      let state = cartReducer(emptyState, cartActions.addItem(makeItem({ quantity: 2, stockLimit: 7 })));
      // addItem without stockLimit — should NOT wipe the existing cap
      state = cartReducer(state, cartActions.addItem(makeItem({ quantity: 1, stockLimit: undefined })));
  ...
  describe('stockLimit cap — updateQuantity', () => {
    it('does not exceed stockLimit when delta pushes quantity over the cap', () => {
      let state = cartReducer(emptyState, cartActions.addItem(makeItem({ quantity: 5, stockLimit: 5 })));
  ...
  describe('stockLimit cap — legacy / uncapped items', () => {
    it('does not cap quantity when stockLimit is undefined (legacy items remain uncapped)', () => {
      let state = cartReducer(emptyState, cartActions.addItem(makeItem({ quantity: 99, stockLimit: undefined })));
  ...
  describe('dismissUnavailableRemoved', () => {
    it('resets unavailableRemoved to an empty array', () => {
      let state = cartReducer(emptyState, cartActions.setUnavailableRemoved([makeItem({ id: 'gone' })]));
